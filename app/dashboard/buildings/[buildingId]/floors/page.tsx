    ...
    <div>
      ...
      {floors.length === 0 ? (
        ...
        <div className="space-y-3">
          {floors.map((floor) => (
            <Card key={floor.id} className="flex items-center gap-6">
              <div className="w-12 h-12 bg-[#141414] text-white rounded-2xl flex items-center justify-center font-mono font-bold text-lg shrink-0">
                {floor.levelNumber}
              </div>
              <div className="flex-1">
                <p className="font-bold">{floor.name}</p>
                {floor.description && <p className="text-sm text-slate-500">{floor.description}</p>}
              </div>
